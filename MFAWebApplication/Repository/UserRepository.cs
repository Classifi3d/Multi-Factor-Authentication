...
private string PasswordHashing(string inputString)
{
    ...
    public async Task<LoginSecurityDTO> LoginAsync(UserLoginDTO userLoginDTO)
    {
        ...
        if (user == null || PasswordHashing(userLoginDTO.Password) != user.Password)
        {
        ...
        {
            string token = CreateToken(user.Id); // Generate JWT token
    ...
    public async Task<string?> VerifyMfaAsync(MfaVerificationDTO verificationDTO)
    {
        ...
        return CreateToken(user.Id); // Return JWT token
    }
